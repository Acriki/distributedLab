...
func reducer(task *Task, reducef func(string, []string) string) {
	...
	intermediate := *readFromLocalFile(task.Input)
	//根据kv排序
...
func mapper(task *Task, mapf func(string, string) []KeyValue) {
	//从文件名读取content
	content, err := ioutil.ReadFile(task.Input[0])
	if err != nil {
		log.Fatal("Failed to read file: "+task.Input[0], err)
	}
	//将content交给mapf，缓存结果
	intermediates := mapf(task.Input[0], string(content))
	...
	for i := 0; i < task.NReduce; i++ {
		mapOutput = append(mapOutput, writeToLocalFile(task.TaskNum, i, &buffer[i]))
	}
	//R个文件的位置发送给master
	task.Output = mapOutput
	TaskCompleted(task)
}
...
func getTask() Task {
	...
	args := ExampleArgs{}
	reply := Task{}
	call("Master.AssignTasks", &args, &reply)
	return reply
}
